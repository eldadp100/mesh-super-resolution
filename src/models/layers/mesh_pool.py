...
class MeshPool(nn.Module):
    ...
    def forward(self, fe, meshes, random_pool=False):
        if not random_pool:
            ...
            for mesh_index in range(len(meshes)):
                self.__pool_main(mesh_index, random_pool=True)

    def __pool_main(self, mesh_index, random_pool=False):
        ...
        if not random_pool:
            ...
            edge_groups = MeshUnion(mesh.edges_count, 'cpu')

        while mesh.edges_count > self.__out_target:
            if not random_pool:
                _, edge_id = heappop(queue)
            else:
                edge_id = np.random.randint(0, len(mesh.edges))
            edge_id = int(edge_id)
            if mask[edge_id]:
