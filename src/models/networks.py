...
class MeshEncoderDecoder(nn.Module):
    ...
    def __init__(self, pools, down_convs, up_convs, blocks=0, transfer_data=True):
        ...
        unrolls.reverse()
        self.linear = torch.nn.Linear(down_convs[-1], 3)

    def forward(self, x, meshes):
        fe = self.encoder((x, meshes))
        return self.linear(fe.permute(0, 2, 1))
...
class UNETMeshSuperResolutionNetwork(nn.Module):

    def __init__(self, opt):
        super(UNETMeshSuperResolutionNetwork, self).__init__()

        down_convs = [opt.input_nc] + opt.ncf
        up_convs = opt.ncf[::-1] + [1]
        pool_res = [opt.ninput_edges] + opt.pool_res
        self.unet = MeshEncoderDecoder(pool_res, down_convs, up_convs, blocks=opt.resblocks,
                                       transfer_data=True)

        # self.vertices_movements_network = nn.Sequential(
        #     nn.Linear(ncf[-1], 32),
        #     nn.Linear(32, 32),
        #     nn.Linear(32, 3)
        # )

    def forward(self, fes, meshes):
        out = []
        for i in range(len(meshes)):
            ...
            mesh = _mesh
            edges_moves = self.unet(fe.unsqueeze(0), [mesh])[0]
            new_vs = _mesh.vs.cuda()
            for j, edge in enumerate(mesh.edges):
                new_vs[edge[0]] = new_vs[edge[0]] + edges_moves[j]
                new_vs[edge[1]] = new_vs[edge[1]] + edges_moves[j]
            _mesh.vs = new_vs
            out.append(_mesh)
        return out
